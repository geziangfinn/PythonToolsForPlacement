...
print(placement.coreRegionWidth,placement.coreRegionHeight)

if placement.coreRegionWidth < placement.coreRegionHeight:
    imageHeight = 1.0 * placement.coreRegionHeight / (placement.coreRegionWidth / minImgaeLength)
    imageWidth = minImgaeLength
else:

    imageWidth = 1.0 * placement.coreRegionWidth / (placement.coreRegionHeight / minImgaeLength)
    imageHeight = minImgaeLength


unitX = imageWidth / placement.coreRegionWidth
unitY = imageHeight / placement.coreRegionHeight



# todo: only calculate modules in coreRegion! 
...
for cell_name in placement.cells.keys():
    ...
    if placement.cells[cell_name].left_x<placement.coreRegion_left_x: 
        continue
    if placement.cells[cell_name].left_x>placement.coreRegion_right_x-placement.cells[cell_name].width/2:
        continue
    if placement.cells[cell_name].low_y<placement.coreRegion_low_y: 
        continue
    if placement.cells[cell_name].low_y>placement.coreRegion_high_y-placement.cells[cell_name].height/2:
        continue

    counter+=1
    
    scaledLeftX=int((placement.cells[cell_name].left_x-placement.coreRegion_left_x)*unitX)
    scaledRightX=int((placement.cells[cell_name].right_x-placement.coreRegion_left_x)*unitX)
    # scaledLowY=int((placement.coreRegionHeight-(placement.cells[cell_name].high_y-placement.coreRegion_low_y))*unitY)
    # scaledHighY=int((placement.coreRegionHeight-(placement.cells[cell_name].low_y-placement.coreRegion_low_y))*unitY)

    scaledLowY=int((placement.cells[cell_name].low_y-placement.coreRegion_low_y)*unitY)
    scaledHighY=int((placement.cells[cell_name].high_y-placement.coreRegion_low_y)*unitY)
    ...
    subImg=img[ps[0]:pe[0],ps[1]:pe[1]]
...
bin_length_x=placement.coreRegionWidth/bin_dim_x
bin_length_y=placement.coreRegionHeight/bin_dim_y
...
for cell_name in placement.cells.keys():
    ...
    if placement.cells[cell_name].left_x<placement.coreRegion_left_x: 
        continue
    if placement.cells[cell_name].left_x>placement.coreRegion_right_x-placement.cells[cell_name].width:
        continue
    if placement.cells[cell_name].low_y<placement.coreRegion_low_y: 
        continue
    if placement.cells[cell_name].low_y>placement.coreRegion_high_y-placement.cells[cell_name].height:
        continue

    bin_start_x=int((placement.cells[cell_name].left_x-placement.coreRegion_left_x)/bin_length_x)
    bin_end_x=int((placement.cells[cell_name].right_x-placement.coreRegion_left_x)/bin_length_x)
    bin_start_y=int((placement.cells[cell_name].low_y-placement.coreRegion_low_y)/bin_length_y)
    bin_end_y=int((placement.cells[cell_name].high_y-placement.coreRegion_low_y)/bin_length_y)
...
# ! plus coreRegion left!!!
        bin_ij_left_x=i*bin_length_x+placement.coreRegion_left_x
        bin_ij_right_x=(i+1)*bin_length_x+placement.coreRegion_left_x
...
# ! plus coreRegion low!!!
            bin_ij_low_y=j*bin_length_y+placement.coreRegion_low_y
            bin_ij_high_y=(j+1)*bin_length_y+placement.coreRegion_low_y
